...
class LiteLLMGatewayBackend:
    ...
    async def usage_snapshot(self) -> UsageSnapshot:
        # Local has no quota; the dispatch layer should never skip this
        # backend on cooldown/exhaustion grounds. Reporting a tiny non-zero
        # remaining_fraction keeps it eligible without competing with Codex
        # for "primary" status (selector prefers larger remaining_fraction).
