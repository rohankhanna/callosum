...
def test_usage_endpoint_returns_identifiable_cached_uncached_rate_table(tmp_path: Path) -> None:
    ...
    assert body["unit"] == "weekly_used_percent"
    assert body["basis"] == "empirical_request_log"
    assert set(body["meters"]) == {"five_hourly", "weekly"}
    assert body["meters"]["five_hourly"]["unit"] == "five_hourly_used_percent"
    assert body["meters"]["weekly"]["unit"] == "weekly_used_percent"
    row = _rate(body)
    assert row["samples"] == {"total": 5, "usable": 5}
    assert row["input_uncached"]["available"] is True
    assert row["input_cached"]["available"] is True
    assert row["output"]["available"] is True
    assert row["reasoning"]["available"] is True
    assert row["cache_effect"]["available"] is True
    assert row["input_uncached"]["rate"] > row["input_cached"]["rate"]
    assert row["source"] == "measured_from_weekly_quota_log"
    assert body["metadata"]["cost_label_attribution"]["included_serialized"] == 5
    assert body["metadata"]["cost_label_attribution"]["overlapped"] == 0
    assert body["metadata"]["cost_label_attribution"]["attribution_key_scope"] == "backend_level_serialization"
...
def test_usage_endpoint_accumulates_zero_delta_rows_until_integer_tick(tmp_path: Path) -> None:
    usage_log = _usage_log(tmp_path)
    for i in range(4):
        _insert_row(
            usage_log.path,
    ...
    assert row["samples"] == {"total": 5, "usable": 1}
    assert row["input_uncached"]["note"] == "too_few_verifiable_samples"


def test_usage_endpoint_accumulates_zero_delta_five_hourly_rows_until_integer_tick(tmp_path: Path) -> None:
    usage_log = _usage_log(tmp_path)
    for i in range(4):
        _insert_row(
            usage_log.path,
    ...
    row = _rate(body["meters"]["five_hourly"])
    assert row["samples"] == {"total": 5, "usable": 1}
    assert row["input_uncached"]["note"] == "too_few_verifiable_samples"


def test_usage_endpoint_reports_different_weekly_and_five_hourly_rates(tmp_path: Path) -> None:
    usage_log = _usage_log(tmp_path)
    for i, uncached in enumerate([100, 200, 300, 400, 500]):
        _insert_row(
            usage_log.path,
            model="model-a0e7",
            effort="medium",
    ...
    weekly = _rate(body["meters"]["weekly"])
    five_hourly = _rate(body["meters"]["five_hourly"])
...
def test_usage_endpoint_marks_insufficient_data_honestly(tmp_path: Path) -> None:
    usage_log = _usage_log(tmp_path)
    _insert_row(
        usage_log.path,
        model="model-a0e7",
    ...
    with TestClient(create_app(backends=[backend], usage_log=usage_log)) as client:
        row = _rate(client.get("/v1/usage").json(), effort="high")

    assert row["samples"] == {"total": 1, "usable": 1}
    assert row["input_uncached"]["available"] is False
    assert row["input_uncached"]["note"] == "too_few_verifiable_samples"
    assert row["confidence"] == "insufficient_data"
...
def test_usage_endpoint_reports_overlapped_label_counts(tmp_path: Path) -> None:
    usage_log = _usage_log(tmp_path)
    for _i in range(5):
        _insert_row(
            usage_log.path,
    ...
    row = _rate(body)
    assert row["samples"] == {"total": 5, "usable": 0}
    assert body["metadata"]["cost_label_attribution"]["overlapped"] == 5
    assert body["metadata"]["cost_label_attribution"]["included_serialized"] == 0
