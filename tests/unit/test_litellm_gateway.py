...
async def test_chat_completions_forwards_large_tool_request_no_size_cap() -> None:
    # No byte-size pre-flight cap: a large tool request must reach the gateway
    # (here a 500) instead of being rejected with 413 before send.
    calls: list[str] = []
...
def _collect_stream_events(raw_iter_chunks: list[bytes]) -> list[dict[str, Any]]:
    events: list[dict[str, Any]] = []
    for raw in raw_iter_chunks:
        for ev_chunk in raw.split(b"\n\n"):
            for line in ev_chunk.split(b"\n"):
                if line.startswith(b"data:"):
                    s = line[5:].strip().decode()
...
async def test_responses_stream_inband_tag_straddling_deltas_routes_to_reasoning(monkeypatch) -> None:
    """ACCEPTANCE: for an `inband_tags` cell, a streamed turn where a
    `<think>` tag straddles two content deltas emits a reasoning item +
    reasoning_summary_text.delta for the CoT, and output_text carries
    ONLY the final answer (tags stripped)."""
    # Simulate the capability gate classifying this cell as inband_tags.
    from callosum.transforms.inband_reasoning import InbandReasoningSplitter

    monkeypatch.setattr(
        "callosum.transforms.inband_reasoning.inband_splitter_for_model",
        lambda model, **kw: InbandReasoningSplitter(),
    )

    # The opening tag straddles deltas 1/2; the closing tag straddles 3/4.
    chunks = [
        b'data: {"id":"x","model":"model-a0d5","choices":[{"delta":{"content":"<thi"}}]}\n\n',
        b'data: {"id":"x","choices":[{"delta":{"content":"nk>the ball is $0.05"}}]}\n\n',
        b'data: {"id":"x","choices":[{"delta":{"content":"</thi"}}]}\n\n',
        b'data: {"id":"x","choices":[{"delta":{"content":"nk>The ball costs $0.05."}}]}\n\n',
        b'data: {"id":"x","choices":[{"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":9,"completion_tokens":7,"total_tokens":16}}\n\n',
        b"data: [DONE]\n\n",
    ]
    ...
    # The visible answer carries ONLY the final answer — no tags leaked.
    text_deltas = [e for e in events if e["type"] == "response.output_text.delta"]
    visible = "".join(e["delta"] for e in text_deltas)
    assert visible == "The ball costs $0.05."
    assert "<think>" not in visible and "</think>" not in visible

    # response.completed reflects the split: reasoning item + clean message.
    completed = [e for e in events if e["type"] == "response.completed"][0]["response"]
    item_types = [o["type"] for o in completed["output"]]
    assert "reasoning" in item_types and "message" in item_types
    msg = [o for o in completed["output"] if o["type"] == "message"][0]
    assert msg["content"][0]["text"] == "The ball costs $0.05."
    await backend.aclose()


async def test_responses_stream_no_splitter_is_byte_for_byte_unchanged(monkeypatch) -> None:
    """For a non-inband cell the gate returns None and content flows
    verbatim — literal `<` in content is preserved, not treated as a tag."""
    monkeypatch.setattr(
        "callosum.transforms.inband_reasoning.inband_splitter_for_model",
        lambda model, **kw: None,
    )
    chunks = [
        b'data: {"id":"x","model":"model-a0d5","choices":[{"delta":{"content":"a < b"}}]}\n\n',
    ...
    events = _collect_stream_events(raw_chunks)
    text_deltas = [e for e in events if e["type"] == "response.output_text.delta"]
    assert "".join(e["delta"] for e in text_deltas) == "a < b"
