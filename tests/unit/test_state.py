...
def test_save_is_atomic_on_overwrite(tmp_path: Path) -> None:
    store = StateStore(tmp_path)
    first = UsageSnapshot(
        remaining_fraction=0.9, cooldown_until_ts=None, weekly_exhausted=False, probed_at_ts=1.0
    )
    second = UsageSnapshot(
        remaining_fraction=0.1, cooldown_until_ts=10.0, weekly_exhausted=True, probed_at_ts=2.0
    )
