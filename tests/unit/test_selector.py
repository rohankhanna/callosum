...
def _fake(
    ...
    *,
    ...
    return InMemoryFakeBackend(
        id=id,
        advertised_models=advertised_models or frozenset({"model-a0f5-mini"}),
        health=health,
        usage=usage,
    )
...
async def test_empty_pool_returns_none() -> None:
    assert await select([], model="model-a0f5-mini") is None


async def test_single_healthy_backend_is_chosen() -> None:
    backend = _fake("a")
    assert await select([backend], model="model-a0f5-mini") is backend


async def test_backend_without_model_is_skipped() -> None:
    backend = _fake("a", advertised_models=frozenset({"other-model"}))
    assert await select([backend], model="model-a0f5-mini") is None
...
async def test_backend_in_cooldown_is_skipped() -> None:
    ...
    assert await select([backend], model="model-a0f5-mini", now_ts=50.0) is None


async def test_backend_past_cooldown_is_included() -> None:
    backend = _fake("a", usage=_usage(cooldown_until=100.0))
    assert await select([backend], model="model-a0f5-mini", now_ts=200.0) is backend
...
async def test_weekly_exhausted_is_deprioritized() -> None:
    ...
    assert await select([a, b], model="model-a0f5-mini") is b
...
async def test_none_remaining_is_treated_as_half() -> None:
    ...
    assert await select([low, unknown], model="model-a0f5-mini") is unknown
...
async def test_recency_breaks_ties_when_usage_equal() -> None:
    ...
    assert await select([stale, fresh], model="model-a0f5-mini") is fresh
...
async def test_id_is_final_deterministic_tiebreak() -> None:
    ...
    assert await select([a, b], model="model-a0f5-mini") is a
...
async def test_all_unhealthy_returns_none() -> None:
    ...
    assert await select([a, b], model="model-a0f5-mini") is None
...
async def test_mixed_pool_picks_best_eligible() -> None:
    ...
    chosen = await select(pool, model="model-a0f5-mini")
