...
| **Coverage vs cost-optimal routing** | *Cost-optimal routing* = route to the cheapest cell believed good enough now. *Coverage* = deliberately route to under-sampled cells to *measure* their quality. You cannot route "cheap when it's just as good" until coverage has *measured* "just as good." Quality data is the price of efficiency, paid up front. |
| **Minimum-coverage quota** | A per-cell minimum-usage floor on organic traffic — a deterministic routing rule (not a self-tuning system) that forces under-covered cells to get a bootstrap/maintenance share of eligible turns so quality labels keep accruing across the whole grid. The live replacement for the removed synthetic tier. Two knobs (budget, window) and an on/off switch. See `docs/architecture/min_coverage_quota.md`. |
