...
def write_index(functions: dict[str, FunctionDefn], edges: dict[str, set[str]]) -> None:
    ...
    md = [
        ...
        (
            "These graphs are review aids: dynamic dispatch, closures, FastAPI decorators, "
            "and protocol calls are necessarily approximate."
        ),
        "",
        "## Artifacts",
        "",
        (
            "- `call_graph_focus.dot` / `call_graph_focus.svg` / `call_graph_focus.json` - "
            "runtime-root call graph, five static call levels deep."
        ),
        (
            "- `control_flow_focus.dot` / `control_flow_focus.svg` / `control_flow_focus.json` - "
            "statement-level control-flow sketches for dispatch/router roots."
        ),
        "",
        "## Focus Roots",
        "",
    ...
    md.extend(
        [
            ...
            "",
            (
                "- `DRILL-001` Request dispatch and retry: start at "
                "`callosum.app:_dispatch_internal`, then `_dispatch_*_with_cell_retry`, "
                "`_dispatch_nonstream`, and `_dispatch_stream`."
            ),
            (
                "- `DRILL-002` Router decision correctness: start at "
                "`callosum.routing.router:Router.route`, then `extract_features`, "
                "`CapabilityFilter.filter`, predictor, and selector nodes."
            ),
            (
                "- `DRILL-003` Observability and state persistence: start at "
                "`callosum.app:_log_attempt` and follow usage-log, estimator-finalize, "
                "peer-quality, and failure-registry calls."
            ),
            (
                "- `DRILL-004` Startup/background behavior: start at "
                "`callosum.__main__:_run_server`, `callosum.app:create_app`, `lifespan`, "
                "smoke tester, cooldown prober, catalog refresh, and capability harness scheduling."
            ),
            "",
        ]
    )
